...
from app.workshops.dtos.workshop_dto import (
    WorkshopResponse, WorkshopRegisterPublic, WorkshopUpdate, WorkshopAdminUpdate
)
from app.workshops.services.workshop_service import WorkshopService
...
def get_all_workshops(
    ...
    current_user=Depends(require_permission("workshops:read")),
):
...
def get_my_workshop(
    ...
    _auth=Depends(require_permission("workshops:read")),
):
...
def update_my_workshop(
    ...
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    _auth=Depends(require_permission("workshops:update")),
):
...
def delete_workshop(
    workshop_id: uuid.UUID,
    db: Session = Depends(get_db),
    _auth=Depends(require_permission("workshops:delete")),
):
